...
def _ler_microareas(cidade):
    """
    Lê Microáreas.xlsx e filtra pelo município.
    pd.read_excel já converte os valores para float corretamente — não aplicar _br_to_float.
    """
    ...
    # Garante que as colunas numéricas são float (já vêm convertidas pelo read_excel)
    cols_numericas = [
        MICROAREAS_COL_POPULACAO,
        MICROAREAS_COL_POP_ATE9, MICROAREAS_COL_POP_10_14, MICROAREAS_COL_POP_15_19,
    ] + [MICROAREAS_COL_POP_CLASSE_PREFIX + c for c in MICROAREAS_POP_CLASSES]
...
def _matriculas_por_bairro(cidade, ano, bairros="automatico", top_n=6):
    ...
    try:
        ...
        if not df_demo.empty:
            ...
            # Fórmula: Pop_classe / Pop_total * 100
            # Colunas fonte: "População por Faixa de Renda vs Faixa Etária - A++" etc.
            POP_PREFIX = MICROAREAS_COL_POP_CLASSE_PREFIX
            CLASSES_SLIDE = ["A++", "A+", "B1", "B2", "C1", "C2", "D", "E"]
            CLASSES_KEYS  = ["pct_classe_App","pct_classe_Ap","pct_classe_B1",
                              "pct_classe_B2","pct_classe_C1","pct_classe_C2",
                              "pct_classe_D_raw","pct_classe_E_raw"]

            pop_col = MICROAREAS_COL_POPULACAO  # denominador = população total

            for classe, key in zip(CLASSES_SLIDE, CLASSES_KEYS):
                col_bruta = POP_PREFIX + classe
                if col_bruta in df_demo.columns:
                    df_demo[key] = df_demo.apply(
                        lambda r, c=col_bruta: fmt_pct(r[c], r[pop_col]), axis=1
                    )

            # D/E = (pop_D + pop_E) / pop_total
            col_d = POP_PREFIX + "D"
            col_e = POP_PREFIX + "E"
            if col_d in df_demo.columns and col_e in df_demo.columns:
                df_demo["pct_classe_D_E"] = df_demo.apply(
                    lambda r: fmt_pct(r[col_d] + r[col_e], r[pop_col]), axis=1
                )
            else:
                df_demo["pct_classe_D_E"] = "—"

            # Também guarda proporção 0-1 para alimentar os charts 2-7 do slide 2
            for classe, key in zip(CLASSES_SLIDE, CLASSES_KEYS):
                col_bruta = POP_PREFIX + classe
                key_ratio = key.replace("pct_","ratio_")
                if col_bruta in df_demo.columns:
                    df_demo[key_ratio] = df_demo.apply(
                        lambda r, c=col_bruta: (r[c] / r[pop_col]) if r[pop_col] > 0 else 0, axis=1
                    )
            col_d_e_ratio = "ratio_classe_D_E"
            if col_d in df_demo.columns and col_e in df_demo.columns:
                df_demo[col_d_e_ratio] = df_demo.apply(
                    lambda r: ((r[col_d] + r[col_e]) / r[pop_col]) if r[pop_col] > 0 else 0, axis=1
                )

            # Seleciona colunas para merge (pct para tabela + ratio para charts)
            ...
            cols_ratio = [c for c in ["ratio_classe_App","ratio_classe_Ap","ratio_classe_B1","ratio_classe_B2",
                                      "ratio_classe_C1","ratio_classe_C2","ratio_classe_D_E"] if c in df_demo.columns]
            cols_sel += cols_pct + cols_ratio
...
def _buscar_dados_reais(cidade, ano_pontual, anos_evolucao, anos_cagr, bairros):
    ...
    for _, row in df_bairros.iterrows():
        ...
        # Classes sociais — % texto para tabela e ratio 0-1 para charts
        ...
        # Ratios 0-1 para os charts 2-7 (linha A++ a D/E)
        # Ordem: primeiro ponto idx=0 é "Renda" (ignorado), idx=1..7 = A++ A+ B1 B2 C1 C2 D/E
        entry["ratios_classes"] = [
            row.get("ratio_classe_App",   0),
            row.get("ratio_classe_Ap",    0),
            row.get("ratio_classe_B1",    0),
            row.get("ratio_classe_B2",    0),
            row.get("ratio_classe_C1",    0),
            row.get("ratio_classe_C2",    0),
            row.get("ratio_classe_D_E",   0),
        ]
